...
export class SendQrScanNotificationUsecase {
  ...
  async execute(token: string) {
                    ...
                    <table width="100%" cellpadding="0" cellspacing="0" border="0">
                      <tr>
                        <td align="center" style="padding: 10px 0 30px 0;">
                          <a href="${process.env.FRONTEND_URL || '#'}/dashboard" style="display: inline-block; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: #ffffff; font-size: 16px; font-weight: 600; text-decoration: none; padding: 14px 40px; border-radius: 8px; box-shadow: 0 4px 15px rgba(102, 126, 234, 0.4);">
                            View Dashboard
                          </a>
                        </td>
                      </tr>
    ...
    await this.mailer.send(
      email,
      `🔔 QR Code Scanned: ${item.item_name}`,
      html
    );
  }
}